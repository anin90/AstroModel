...
pdf("bd_tbl_abs/bd_tbl_abs.pdf")

###########
# Load data
###########

bd_209_tbl <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_abs/bd_209_tbl_abs.csv",
		header = T, sep = "\t")
bd_r_72_tbl <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_abs/bd_r_72_tbl_abs.csv",
		header = T, sep = "\t")		
bd_nr_733_tbl <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_abs/bd_nr_733_tbl_abs.csv",
		header = T, sep = "\t")
...
# Fig.1: venn rxns
	x <- list(
		...
		BD_NonResponder = bd_nr_733_tbl$rxnList)
...
# Fig.2: venn subsystems
	x <- list(
		...
		BD_NonResponder = bd_nr_733_tbl$subSystem)
...
# Fig.3: Number of rxns per subSystem (bd_lumped)
	...
	comb = merge(bd_209_tbl_subSystem, iPS_BD_TP_subSystem, by.x = "subSystem", by.y = "SubSystem")
	...
	write.table(bd_lumped_subsystem_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant_abs/bd_lumped_subsystem_fdr_abs.csv", sep = "\t", quote = FALSE, row.names = TRUE, col.names=NA)
	bd_lumped_rxns_fdr = bd_209_tbl[bd_209_tbl$subSystem %in% bd_lumped_subsystem_fdr$subSystem,]
	write.table(bd_lumped_rxns_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant_abs/bd_lumped_rxns_fdr_abs.csv", sep = "\t", quote = FALSE, row.names = TRUE, col.names=NA)
...
# Fig.4: Number of rxns per subSystem (bd_responder)
	...
	comb = merge(bd_r_72_tbl_subSystem, iPS_BD_R_TP_subSystem, by.x = "subSystem", by.y = "SubSystem")
	comb = comb[order(comb$n.x),]
	#4a
	dotchart(comb$n.x, labels = comb$subSystem,
	...
	write.table(bd_r_subsystem_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant_abs/bd_r_subsystem_fdr_abs.csv", sep = "\t", quote = FALSE, row.names = TRUE, col.names=NA)
	bd_r_rxns_fdr = bd_r_72_tbl[bd_r_72_tbl$subSystem %in% bd_r_subsystem_fdr$subSystem,]
	write.table(bd_r_rxns_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant_abs/bd_r_rxns_fdr_abs.csv", sep = "\t", quote = FALSE, row.names = TRUE, col.names=NA)
...
# Fig.5: Number of rxns per subSystem (bd_nonresponder)
	...
	bd_nr_733_tbl_subSystem = bd_nr_733_tbl %>% count(subSystem)
	comb = merge(bd_nr_733_tbl_subSystem, iPS_BD_NR_TP_subSystem, by.x = "subSystem", by.y = "SubSystem")
	comb = comb[order(comb$n.x),]
	#5a
	dotchart(comb$n.x, labels = comb$subSystem,
			cex = 0.7, bg = "red", xlab = "# of rxns", main = "# of disrupted rxns (n=733, bd_nonresponder)")	
	#5b
	dotchart(comb$n.y, labels = comb$subSystem,
			cex = 0.7, bg = "grey", xlab = "# of rxns", main = "# of disrupted rxns (n=733, bd_nonresponder) vs all rxns")	
	...
	write.table(bd_nr_subsystem_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant_abs/bd_nr_subsystem_fdr_abs.csv", sep = "\t", quote = FALSE, row.names = TRUE, col.names=NA)
	bd_nr_rxns_fdr = bd_nr_733_tbl[bd_nr_733_tbl$subSystem %in% bd_nr_subsystem_fdr$subSystem,]
	write.table(bd_nr_rxns_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant_abs/bd_nr_rxns_fdr_abs.csv", sep = "\t", quote = FALSE, row.names = TRUE, col.names=NA)

# Fig.6: Number of rxns per compartment (bd_lumped)
	#6a: all.209
	myTable <- table(bd_209_tbl$Flux, bd_209_tbl$Localization)
...
# Fig.7: Number of rxns per compartment (bd_responder)
	#7a: all.72
	myTable <- table(bd_r_72_tbl$Flux, bd_r_72_tbl$Localization)
...
# Fig.8: Number of rxns per compartment (bd_nonresponder)
	#8a: all.733
	myTable <- table(bd_nr_733_tbl$Flux, bd_nr_733_tbl$Localization)
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		geom_text(data=subset(as.data.frame(myTable),Freq != 0), size = 3, position = position_stack(vjust = 0.5)) +
		theme(axis.text.x = element_text(size=12, angle = 60, hjust=1))+ 
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (all, n=733) per compartment (bd_nonresponder)")
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		geom_text(data=subset(as.data.frame(myTable),Freq != 0), size = 3, position = position_stack(vjust = 0.5)) +
		theme(axis.text.x = element_text(size=12, angle = 60, hjust=1)) + 
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (fdr.significant, n=257) per compartment (bd_nonresponder)")
