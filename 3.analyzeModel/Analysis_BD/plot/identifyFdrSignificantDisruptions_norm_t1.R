...
# Fig.1: venn rxns
	x <- list(
		BD_Lumped = bd_199_tbl$rxnList, 
		BD_Responder = bd_r_185_tbl$rxnList, 
		BD_NonResponder = bd_nr_184_tbl$rxnList)
	ggvenn(x, fill_color = c("blue", "green", "red")) + ggtitle("Overlap in rxns disrupted between models")

# Fig.2: venn subsystems
	x <- list(
		BD_Lumped = bd_199_tbl$subSystem, 
		BD_Responder = bd_r_185_tbl$subSystem, 
		BD_NonResponder = bd_nr_184_tbl$subSystem)
...
# Fig.6: Number of rxns per compartment (bd_lumped)
	#6a: all.199
	myTable <- table(bd_199_tbl$Flux, bd_199_tbl$Localization)
...
# Fig.7: Number of rxns per compartment (bd_responder)
	#7a: all.185
	myTable <- table(bd_r_185_tbl$Flux, bd_r_185_tbl$Localization)
...
# Fig.8: Number of rxns per compartment (bd_nonresponder)
	#8a: all.184
	myTable <- table(bd_nr_184_tbl$Flux, bd_nr_184_tbl$Localization)
