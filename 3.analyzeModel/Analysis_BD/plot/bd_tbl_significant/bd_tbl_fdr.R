...
pdf("bd_tbl_fdr.pdf", width=7, height=3)
...
# load subsystems
bd_lumped_subsystem_fdr <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant/bd_lumped_subsystem_fdr.csv",
		header = T, sep = "\t")
bd_r_subsystem_fdr <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant/bd_r_subsystem_fdr.csv",
		header = T, sep = "\t")		
bd_nr_subsystem_fdr <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant/bd_nr_subsystem_fdr.csv",
		header = T, sep = "\t")

# load rxns
bd_lumped_rxns_fdr <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant/bd_lumped_rxns_fdr.csv",
		header = T, sep = "\t")
bd_r_rxns_fdr <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant/bd_r_rxns_fdr.csv",
		header = T, sep = "\t")		
bd_nr_rxns_fdr <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/Analysis_BD/plot/bd_tbl_significant/bd_nr_rxns_fdr.csv",
		header = T, sep = "\t")
...
# Fig.1a: Number of rxns per subSystem post fdr (BD_Lumped)
	...
	# Fig.1b:
	myTable <- table(bd_lumped_rxns_fdr$Var7, bd_lumped_rxns_fdr$subSystem)
	head(myTable)
...
# Fig.2: Number of rxns per subSystem post fdr (BD_Responder)
	...
	# Fig.2b:
	myTable <- table(bd_r_rxns_fdr$Var7, bd_r_rxns_fdr$subSystem)
	head(myTable)
	ggplot(as.data.frame(myTable), aes(x = reorder(Var2, -Freq), Freq, fill=Var1, label = Freq))+ 
...
# Fig.3: Number of rxns per subSystem post fdr (BD_NonResponder)
	...
	# Fig.3b:
	myTable <- table(bd_nr_rxns_fdr$Var7, bd_nr_rxns_fdr$subSystem)
	head(myTable)
	ggplot(as.data.frame(myTable), aes(x = reorder(Var2, -Freq), Freq, fill=Var1, label = Freq))+ 
		geom_bar(stat="identity") + coord_flip() + 
