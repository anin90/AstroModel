...
# Fig.3: Number of rxns per subSystem (bd_lumped)
	...
	dotchart(comb$n.x, labels = comb$subSystem,
			cex = 0.7, bg = "blue", xlab = "# of rxns", main = "# of disrupted rxns (n=212, bd_lumped)")
	#3b
	dotchart(comb$n.y, labels = comb$subSystem,
			cex = 0.7, bg = "grey", xlab = "# of rxns", main = "# of disrupted rxns (n=212, bd_lumped) vs all rxns")	
...
# Fig.4: Number of rxns per subSystem (bd_responder)
	...
	#4a
	dotchart(comb$n.x, labels = comb$subSystem,
			cex = 0.7, bg = "green", xlab = "# of rxns", main = "# of disrupted rxns (n=92, bd_responder)")
	#4b
	dotchart(comb$n.y, labels = comb$subSystem,
			cex = 0.7, bg = "grey", xlab = "# of rxns", main = "# of disrupted rxns (n=92, bd_responder) vs all rxns")	
...
# Fig.5: Number of rxns per subSystem (bd_nonresponder)
	...
	#5a
	dotchart(comb$n.x, labels = comb$subSystem,
			cex = 0.7, bg = "red", xlab = "# of rxns", main = "# of disrupted rxns (n=670, bd_nonresponder)")	
	#5b
	dotchart(comb$n.y, labels = comb$subSystem,
			cex = 0.7, bg = "grey", xlab = "# of rxns", main = "# of disrupted rxns (n=670, bd_nonresponder) vs all rxns")	
...
# Fig.6: Number of rxns per compartment (bd_lumped)
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		geom_text(data=subset(as.data.frame(myTable),Freq != 0), size = 3, position = position_stack(vjust = 0.5)) +
		theme(axis.text.x = element_text(size=12, angle = 60, hjust=1)) + 
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (fdr.significant, n=95) per compartment (bd_lumped)")
