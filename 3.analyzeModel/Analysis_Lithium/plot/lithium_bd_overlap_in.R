...
pdf("lithium_bd_venn_in.pdf", height=3.5, width=3.5)
...
#1a (venn hypergeometric test, Li+ action-InputRxnsOnly vs BD signature)
	x <- list(Li_Akkouh = Akkouh_in, Li_GSE66276 = GSE66276_in, Li_GSE132397 = GSE132397_in,	
	...
	pheatmap(M, cluster_rows=F, cluster_cols=F, na_col="white", display_numbers = TRUE, main = 'BD vs Li+ (Primary)')
	
	M <- hyper_matrix(x, background);	M[lower.tri(M)] <- NA;	#diag(M)<-NA;
	M
