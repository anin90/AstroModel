...
pdf("st_tbl_norm_t1/st_tbl_norm_t1.pdf")

###########
# Load data
###########

ST <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/2.Koskuvi/FSr_ST/PlotResults/st_tbl_norm_t1/st_rxns.csv",
		header = T, sep = "\t")
...
# Fig.1: Number of rxns per subSystem (st_rxns)
	...
	dotchart(comb$n.x, labels = comb$subSystem,
			cex = 0.7, bg = "blue", xlab = "# of rxns", main = "# of disrupted rxns (n=98, ST)")
	#1b
	dotchart(comb$n.y, labels = comb$subSystem,
			cex = 0.7, bg = "grey", xlab = "# of rxns", main = "# of disrupted rxns (n=98, ST) vs all rxns")	
...
# Fig.2: Number of rxns per compartment (st_rxns)
	...
	compartment <- c("cytoplasm", "extracellular space", "lysosome", "mitochondrion", "nucleus", 
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (all, n=98) per compartment (ST)")
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		geom_text(data=subset(as.data.frame(myTable),Freq != 0), size = 3, position = position_stack(vjust = 0.5)) +
		theme(axis.text.x = element_text(size=12, angle = 60, hjust=1)) + 
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (fdr.significant, n=33) per compartment (ST)")
