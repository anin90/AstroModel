...
%% ASTROCYTE_FUNCTION_TESTS
    ...
    %% FAOXC164C165m - Beta Oxidation of Fatty Acid
    model = modelOri;
    model.c(find(model.c)) = 0;
    model.c(ismember(model.rxns,'FAOXC164C165m'))=1;
    if find(model.c)>0
        FBA = optimizeCbModel(model,'max','zero');
        TestSolution(k,1) = FBA.f;
    else
    ...
    TestSolutionName{k,1} = 'FAOXC164C165m';
    TestSolutionGroup{k,1} = 'Beta Oxidation of Fatty Acid';
    TestedMetabolite{k,1} = 'NA';
    TestedModel{k,1} = 'modelOri';
    k = k +1;   
