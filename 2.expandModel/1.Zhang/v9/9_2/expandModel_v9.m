...
%% specific to v9:
model = addReaction(model,'GLUNm','reactionFormula','gln_L[m] + h2o[m] -> glu_L[m] + nh4[m]', 'subSystem','Glutamate metabolism', 'geneRule','2744.1 or 27165.2 or 27165.1');
% model = addReaction(model,'FOLR2','reactionFormula','fol[c] + nadph[c] -> dhf[c] + nadp[c]', 'subSystem','Folate metabolism', 'geneRule','1719.1');
model = addReaction(model,'CKc','reactionFormula','atp[c] + creat[c] <=> adp[c] + h[c] + pcreat[c]', 'subSystem','Urea cycle', 'geneRule','1152.1 or 1158.1');
...
model = addReaction(model,'CSm','reactionFormula','accoa[m] + h2o[m] + oaa[m] -> cit[m] + coa[m] + h[m]', 'subSystem','Citric acid cycle', 'geneRule','1431.1 or 1431.2');
...
% model = addSinkReactions(model,{'ach[c]'},-10,1000);
