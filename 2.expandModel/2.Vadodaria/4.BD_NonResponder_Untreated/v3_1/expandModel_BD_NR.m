function[model_EXP_Unconstrained, fluxInconsistentRxns_non_media, model_EXP_Constrained, fluxInconsistentRxns_media] = expandModel_BD_NR(model)
tic;
...
model = addReaction(model,'r0963','reactionFormula','fol[e] -> fol[c]', 'subSystem','Transport, extracellular', 'geneRule','2348.1 or 2350.1 or 2352.1'); % wasn't captured by iMAT in BD_R condition
...
model = addReaction(model,'r1116','reactionFormula','atp[c] <=> atp[e]', 'subSystem','Transport, extracellular', 'geneRule',''); % to gapFill EX_atp[e] in BD_R model
model = addReaction(model,'G6PDH2c','reactionFormula','3.0 g6p[c] + 3.0 nadp[c] -> 3.0 6pgl[c] + 3.0 h[c] + 3.0 nadph[c]', 'subSystem','Pentose phosphate pathway', 'geneRule','2539.1'); % to gapFill PGMT in BD_NR model
model = addReaction(model,'HMR_6572','reactionFormula','g6p[c] -> mi3p_D[c]', 'subSystem','Inositol phosphate metabolism', 'geneRule','51477.1'); % to gapFill PGMT in BD_NR model
model = addReaction(model,'UDPGP','reactionFormula','h2o[c] + udpg[c] -> g1p[c] + 2.0 h[c] + ump[c]', 'subSystem','Starch and sucrose metabolism', 'geneRule','5167.1 or 5169.1 or 5168.1'); % to gapFill PGMT in BD_NR model
model = addReaction(model,'r1116','reactionFormula','atp[c] <=> atp[e]', 'subSystem','Transport, extracellular', 'geneRule',''); % to gapFill EX_atp[e] in BD_R model
...
model = addReaction(model,'HYXNt','reactionFormula','hxan[e] <=> hxan[c]', 'subSystem','Transport, extracellular', 'geneRule','3177.1'); % to gapFill HXPRT
