function[geneList, rxnList, FSr_significant_L, FSr_significant_H, FSr_subsystem_L, Subsystem_L_pooled] = downRegulate(model,filename);
tic;
...
delimiterIn = '\t';
geneList = num2cell(importdata('rxnList_PD.csv','\t'));
geneList = cellfun(@num2str,geneList,'uni',0);
geneList = strcat(geneList,'.1');

%% map genes to reactions in model
[rxnList] = findRxnsActiveWithGenes(model, geneList)';
...
FSr_subsystem_L = [FSr_subsystem_L_C];
