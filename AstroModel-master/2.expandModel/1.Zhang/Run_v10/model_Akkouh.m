function[filter_1, filter_2, filter_3, rxnList_down, rxnList_up_f, rxnList_up_b, model_Akkouh, FSr_Akkouh, FSr_significant_L, FSr_significant_H, subSystem_L, subSystem_H, relaxed_AllRxns_LB, relaxed_AllRxns_UB, relaxed_UpRxns_LB, relaxed_UpRxns_UB, relaxed_DownRxns_LB, relaxed_DownRxns_UB, DiffRxns_SetDiff] = expose(model, filename);
tic;
...
% opt_wt = optimizeCbModel(model,'max');
% model = changeRxnBounds(model, 'biomass_maintenance', opt_wt.f/2, 'l');
model = changeRxnBounds(model, 'biomass_maintenance', 1, 'l');
[minFlux_WT, maxFlux_WT] = fluxVariability(model);

deltaModelRelaxed = changeObjective(modelRelaxed, 'biomass_maintenance', 1);
% opt_deltaModelRelaxed = optimizeCbModel(deltaModelRelaxed,'max');
% deltaModelRelaxed = changeRxnBounds(deltaModelRelaxed, 'biomass_maintenance', opt_deltaModelRelaxed.f/2, 'l');
deltaModelRelaxed = changeRxnBounds(deltaModelRelaxed, 'biomass_maintenance', 1, 'l');
