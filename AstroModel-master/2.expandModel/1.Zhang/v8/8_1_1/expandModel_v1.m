...
fluxConsistency = verifyModel(MBA_model_7_3_4,'fluxConsistency',true); fluxInconsistentRxns_non_media = MBA_model_7_3_4.rxns(fluxConsistency.fluxConsistency.consistentReactionBool~=1); clear fluxConsistency;
% fluxInconsistentRxns(find(~cellfun(@isempty,strfind(fluxInconsistentRxns,'[e]'))));

%% save media model
model = changeObjective(model, 'biomass_maintenance', 0);
MBA_model_7_3_4_ASM = model;
