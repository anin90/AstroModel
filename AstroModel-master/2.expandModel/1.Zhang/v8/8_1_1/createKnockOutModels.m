function[rxnList, models_single_KO, model_custom_KO, FSr_single_KO, FSr_custom_KO, FSr_significant, FSr_mat_significant, FSr_significant_subSystems_Pooled, FSr_significant_KO_Specific_subSystems] = KO_model(model,filename);
tic;
...
for a = 1:length(models_single_KO);
    FSr_significant(a).a = models_single_KO{a}.rxns(FSr_single_KO(:,a)<0.8);
    % FSr_significant(a).b = model_custom_KO.rxns(FSr_custom_KO>2 & FSr_custom_KO~=inf);
end;
FSr_significant = struct(FSr_significant); clear a;

% CUSTOM_KO
FSr_significant(1).b = model_custom_KO.rxns(FSr_custom_KO<0.8);
