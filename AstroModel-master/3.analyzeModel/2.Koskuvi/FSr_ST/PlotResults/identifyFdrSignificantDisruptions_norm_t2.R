...
pdf("st_tbl_norm_t2/st_tbl_norm_t2.pdf")

###########
# Load data
###########

ST <- read.csv("/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/2.Koskuvi/FSr_ST/PlotResults/st_tbl_norm_t2/st_rxns.csv",
		header = T, sep = "\t")
...
# Fig.1: Number of rxns per subSystem (st_rxns)
	...
	dotchart(comb$n.x, labels = comb$subSystem,
			cex = 0.7, bg = "blue", xlab = "# of rxns", main = "# of disrupted rxns (n=104, ST)")
	#1b
	dotchart(comb$n.y, labels = comb$subSystem,
			cex = 0.7, bg = "grey", xlab = "# of rxns", main = "# of disrupted rxns (n=104, ST) vs all rxns")	
	...
	write.table(ST_subsystem_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/2.Koskuvi/FSr_ST/PlotResults/st_tbl_significant_norm_t2/ST_subsystem_fdr_norm_t2.csv", sep = "\t", quote = FALSE, row.names = F, col.names=T)
	ST_rxns_fdr = ST[ST$subSystem %in% ST_subsystem_fdr$subSystem,]
	write.table(ST_rxns_fdr, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/2.Koskuvi/FSr_ST/PlotResults/st_tbl_significant_norm_t2/ST_rxns_fdr_norm_t2.csv", sep = "\t", quote = FALSE, row.names = F, col.names=T)
	#write all subsystems to csv
	ST_subsystem_all = combStat
	write.table(ST_subsystem_all, "/media/anirudh/Work/ADBS_NIMHANS/Thesis/1.Science/Analysis/cobratoolbox/AstroModel/3.analyzeModel/2.Koskuvi/FSr_ST/PlotResults/st_tbl_norm_t2/ST_subsystem_all_norm_t2.csv", sep = "\t", quote = FALSE, row.names = F, col.names=T)
...
# Fig.2: Number of rxns per compartment (st_rxns)
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (all, n=104) per compartment (ST)")
	...
	ggplot(as.data.frame(myTable), aes(Var2, Freq, fill=Var1, label = Freq)) + 
		...
		geom_text(data=subset(as.data.frame(myTable),Freq != 0), size = 3, position = position_stack(vjust = 0.5)) +
		theme(axis.text.x = element_text(size=12, angle = 60, hjust=1)) + 
		scale_x_discrete(labels= compartment) + ggtitle("Disrupted rxns (fdr.significant, n=37) per compartment (ST)")
